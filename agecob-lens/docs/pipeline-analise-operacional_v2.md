...
Este é o source of truth conceitual e técnico da área. Os contratos de endpoint e o mapa de KPIs aqui descritos estão programados para fases futuras. Nenhuma query SQL de produção é incluída neste documento para as seções conceituais (mesmo padrão de "placeholder mode" usado em [missing-endpoints-contracts.md](missing-endpoints-contracts.md)) — exceto na seção 9, onde queries validadas em produção são registradas.
...
O redesign do dashboard executivo do dia é definido em [redesign_executivo_dashboard.plan.md](redesign_executivo_dashboard_92b7d68c_plan.md). Ambos os documentos compartilham o mesmo dicionário de métricas e devem ser versionados juntos.
...
Formato seguindo [mapa-kpis-dashboard.md](mapa-kpis-dashboard.md). Queries de produção permanecem como `TODO: BUSINESS_QUERY_REQUIRED` até validação de negócio.
...
Seguindo o mesmo padrão "placeholder mode" de [missing-endpoints-contracts.md](missing-endpoints-contracts.md). SQL permanece `TODO: BUSINESS_QUERY_REQUIRED` exceto onde indicado como validado.
...
> **Nota:** sobreposição intencional com `/dashboard/produtividade-historico/{db}` já definido em [missing-endpoints-contracts.md](missing-endpoints-contracts.md). Decisão pendente: **unificar** em um único endpoint (preferido) ou manter ambos — ver seção 11, item 3.
...
WITH CTE_Acordos AS (
    ...
    WHERE R.DT_EMISSAO >= DATEADD(MONTH, -3, GETDATE())  -- carga inicial; job incremental usa CAST(GETDATE() AS DATE)
      AND R.ID_REC_STATUS IN (1, 3, 11, 12)
...
- [mapa-kpis-dashboard.md](mapa-kpis-dashboard.md) — KPIs do dashboard operacional do dia atual.
- [missing-endpoints-contracts.md](missing-endpoints-contracts.md) — padrão de contrato usado neste documento.
- [data-coverage-analysis.md](data-coverage-analysis.md) — cobertura de dados e análise de gaps.
- [refactor_main_py_report.md](refactor_main_py_report.md) — status atual do backend.
- [redesign_executivo_dashboard.plan.md](redesign_executivo_dashboard_92b7d68c_plan.md) — redesign do dashboard executivo do dia (dicionário de métricas compartilhado e arquitetura de navegação).

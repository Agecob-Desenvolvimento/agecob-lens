...
**Reproduced and precisely diagnosed 2026-08-21** (day after Clusters E/I/J shipped), closing out the earlier flagged-but-undetailed cross-db scope-mixing issue — this is that issue, reproduced fresh with a real repro instead of a memory-only claim.
...
Same mechanism as Cluster G (silent under-sampling presented as complete), one dimension over: there it was "checked 7 of 20 portfolios," here it's "checked 1 of 2 databases." Both stem from the same gap — nothing in `system_prompt.md` requires the agent to disclose *which* scope (portfolios, databases) a consolidated-sounding answer actually covers when it silently narrows from what the session/question implied.

**Not yet fixed.** Given it's the same root cause as Cluster G (already scoped as a Blocker, T3/T4 in the test plan below), the fix likely generalizes: a system-prompt rule requiring explicit scope disclosure ("consultei X de Y carteiras/bancos") whenever an answer doesn't cover the full scope implied by the question or session filters — not a narrow per-tool patch. Folding into T4 rather than opening a separate ticket.
...
- **T4 — Coverage disclosure rule.** Generalizes beyond portfolios (Cluster G) to databases too (Cluster K confirmed 2026-08-21 — same mechanism, `db="todos"` silently narrowed to one bank, off by 3x). Needs a system-prompt rule that covers both dimensions: any answer that doesn't cover the full scope implied by the question/session filters (all active portfolios, both databases when `db="todos"`) must say so explicitly. After it ships, re-run Q2/Q3 (portfolio coverage) and the Cluster K repro (db coverage) — pass criterion is 100% coverage or an explicit "consultei X de Y" statement, no silent partial totals presented as complete.

...
## Known issue not covered above — needs re-derivation before acting on it

Earlier in this session (before the confidence/portfolio-scoping fixes in `f44bfac`), a cross-database scope-mixing bug was found and explained while testing whether the agent's reported numbers matched ground truth for `db="todos"` scenarios. The exact repro steps and numbers were not preserved through this session's context compaction, and per this project's own memory-verification rule, a claim from memory without current evidence is not a claim to act on. **Before this goes into any fix list, reproduce it fresh** — pick 2-3 questions that mix `db="todos"` with per-db totals (e.g. "quanto geramos hoje em COBwebRCBAUTOS vs no total?") and check whether the per-db and `todos` numbers are internally consistent (sum of per-db should equal `todos`, modulo excluded-agent filtering differences that are intentional per `FILTRO_AGENTES_EXCLUIDOS_SQL` vs `FILTRO_AGENTES_EFETIVIDADE_SQL`).
...
- **T4 — Coverage disclosure rule.** After T3 ships, re-run Q2 and Q3 verbatim again. Pass criterion: the agent either covers 100% of active portfolios, or explicitly states how many of how many it covered. No silent partial sums.

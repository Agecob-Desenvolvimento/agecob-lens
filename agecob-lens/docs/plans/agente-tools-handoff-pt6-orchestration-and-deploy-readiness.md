...
| `dominios/agente/conversao.py`'s payload self-labels as "Conversão oficial" | backs `kpi="efetividade"` | ~~Still unfixed~~ — **FIXED in `e6248ee` (2026-09-03)**, after this document was written. The module docstring and `_CRITERIO` now say the opposite: *"NÃO é a Conversão do dicionário (qtd_acordos / qtd_contatos)"* (`dominios/agente/conversao.py:7`, `:35`). `139f9f8` retired the same stale record in pt5 but missed this row; corrected in the 2026-09-16 doc audit. |

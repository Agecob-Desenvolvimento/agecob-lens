...
function buildSteps(d: FunilData): FunilStep[] {
  // Funil: Acionamento → Alô (atende) → Contato/RPC (pessoa certa) → Acordo.
  // Cada taxa é vs a etapa anterior.
    ...
    <Card>
      ...
      <CardContent className="space-y-2.5">
        ...
        <div className="mt-3 flex gap-4 text-[10px] text-slate-500">
          <span>
            Perda Contato→CPC: {lossAloRpc.toFixed(0)}%
          </span>
          <span>
            Perda CPC→Acordos: {lossRpcAcord.toFixed(0)}%
          </span>
        </div>
      </CardContent>
    </Card>

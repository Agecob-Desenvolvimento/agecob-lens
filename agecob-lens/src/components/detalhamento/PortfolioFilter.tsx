...
import type { PortfolioRow } from "@/services/api";
...
export function PortfolioFilter({ portfolios, selected, onSelect, loading }: PortfolioFilterProps) {
  ...
  const selectedName = selected != null
    ? portfolios.find((p) => p.id === selected)?.nome ?? `ID ${selected}`
    : "Todos os portfólios";
    ...
    <div ref={ref} className="relative w-full max-w-[280px]">
      <button
        ...
        className={cn(
          ...
          selected != null ? "font-semibold" : "font-normal",
          loading && "opacity-50 cursor-wait",
        )}
      >
        <Filter className="h-3.5 w-3.5 text-muted-foreground" aria-hidden />
        <span className="flex-1 truncate text-left">{loading ? "Carregando…" : selectedName}</span>
        <ChevronDown
          className={cn(

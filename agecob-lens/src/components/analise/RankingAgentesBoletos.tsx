          ...
          <div className="flex flex-col gap-1.5">
            {data.map((d, i) => (
              <div key={d.nome} className="flex items-center gap-2" data-testid="ranking-row">
                <span className="w-28 shrink-0 truncate text-right text-[11px] text-slate-600 dark:text-slate-300">{d.nome}</span>
                ...
                <span className="w-9 shrink-0 text-right text-[11px] font-semibold tabular-nums text-slate-600 dark:text-slate-300">{d.pct}%</span>
              </div>
            ))}
          </div>
        )}

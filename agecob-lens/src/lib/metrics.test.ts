...
describe("calcConversao", () => {
  it("computes boletos pagos / CPC (qtd_contatos)", () => {
    expect(calcConversao({ qtd_boletos_pagos: 5, qtd_contatos: 50 })).toBe(10);
  });

  it("returns 0 when qtd_contatos is 0", () => {
    expect(calcConversao({ qtd_boletos_pagos: 5, qtd_contatos: 0 })).toBe(0);
  });

  // Regressão ee9699a: fórmula lia qtd_acordos (ausente nos callers) → NaN → freeze no heatmap.
  it("never returns NaN for the caller shape { qtd_boletos_pagos, qtd_contatos }", () => {
    const result = calcConversao({ qtd_boletos_pagos: 3, qtd_contatos: 40 });

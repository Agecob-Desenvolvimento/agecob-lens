...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN CC.ALO = 1 THEN CM.ID_CTO_MASTER END) AS qtd_alo,
    COUNT(DISTINCT CASE WHEN CC.ALO = 1 AND CM.ID_COMPLEMENTO IN {CPC_IDS_SQL} THEN CM.ID_CTO_MASTER END) AS qtd_contatos,
    CAST(
        CEILING(
            COUNT(DISTINCT CASE WHEN CC.ALO = 1 AND CM.ID_COMPLEMENTO IN {CPC_IDS_SQL} THEN CM.ID_CTO_MASTER END) * 100.0
        / NULLIF(COUNT(DISTINCT CASE WHEN CC.ALO = 1 THEN CM.ID_CTO_MASTER END), 0)
        )
...
CTE_Esforco AS (
    SELECT
        ...
        SUM(CM.contato) AS qtd_contatos,
        SUM(CM.alo)     AS qtd_alo
    FROM ({cto_master}) CM

...
| Exceções (`ID_REC_STATUS`) | `= 5` (status PENDENTE) — script de gráficos v2 também usa `(11,)` para exceções de boleto; verificar contexto |
...
### 5. Estilo Caveman (Obrigatório)
Comunicar sempre em estilo Caveman: telegráfico, ultra-compacto, sem pronomes/artigos desnecessários, sem polidez/rodeios, preservando código, caminhos de arquivo, formatação markdown e total precisão técnica para máxima economia de tokens.
